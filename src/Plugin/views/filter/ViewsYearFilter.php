...
class ViewsYearFilter extends InOperator
{
    ...
    public function query()
    {
        ...
        if (!empty($this->value)) {
            ...
            $this->query->addWhereExpression($this->options['group'], "Year($this->tableAlias.$this->realField) = " . $this->value[0]);
    ...
    public function generateOptions()
    {
        ...
        if ($nodes_ids) {
            foreach ($nodes_ids as $node_id) {
                ...
                $parts = explode("-", $node->field_date_publication->value);
